function dataOut = natSc_getData4RCA(database, how, reuse,split)
...
% trial into 4 segments, and uses the specified segment to reconstruct
...
% how.nScenes -- if 1, organize data to subject X condition array cell, if >1 organize data to scenes X condition array cell
    ...
    if how.nScenes == 1
        dataOutFilename = strcat(database, 'data4RCA_', splitcn{:},'_bySubjects', '.mat');
    else
        dataOutFilename = strcat(database, 'data4RCA_', splitcn{:}, '_byScenes','.mat');
    end
    dataOutLocation = fullfile(natSc_path.results_Data, dataOutFilename);
